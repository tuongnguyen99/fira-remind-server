...
function evaluate(req, res) {
    ...
    const query = `INSERT INTO thanhtra (id, m_ttra, giovipham, sisothucte, gv_botiet, gv_ditre, gv_nghisom, gv_saiten, gv_daykhongthongbao, chitiet, id_tkb) VALUES (NULL, '${data.ms_thanhtra}', '${data.giovipham}', '${data.sisothucte}', '${cvt[data.gv_botiet]}', '${cvt[data.gv_ditre]}', '${cvt[data.gv_nghisom]}', '${cvt[data.gv_saiten]}', '${cvt[data.gv_daykhongthongbao]}', '${data.chitiet}', '${data.id_tkb}')`;
    cn.query(query, err => {
        if (err) return res.status(400).send(err.message);
        res.send({
